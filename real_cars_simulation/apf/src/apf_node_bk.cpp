...
int main(int argc, char** argv) {
    ...
    int rotation_count = 0; // max 4 degree;
    bool start_condition = true;
    // float rotation_z[4]  = {0.707, 1.0,  0.707, 0.0}; 
    // float rotation_w[4]  = {0.707, 0.0, -0.707, 1.0}; 
    // float rotation_w[9]  = {0.866, 0.5 ,   0.087,  0.5  ,0.866, 1,   0.866,  0.500, -0.5 } ;
    // float rotation_z[9]  = {0.5  , 0.866 , 0.996, 0.866 ,0.5  , 0,  -0.5,  -0.866, -0.996 };
    ...
    while(ros::ok()){
        ...
        if(start_condition && ac.getState() != actionlib::SimpleClientGoalState::ACTIVE){
            int loc_x, loc_y;
            ...
            loc_x = floor((transform.getOrigin().x()-mapData.info.origin.position.x)/mapData.info.resolution);
            loc_y = floor((transform.getOrigin().y()-mapData.info.origin.position.y)/mapData.info.resolution);
            ...
            if(rotation_count <3){
                robotGoal.target_pose.pose.position.x = loc_x*mapData.info.resolution + mapData.info.origin.position.x-0.5;
                robotGoal.target_pose.pose.position.y = loc_y*mapData.info.resolution + mapData.info.origin.position.y-0.5;
            }
            else{
                robotGoal.target_pose.pose.position.x = loc_x*mapData.info.resolution + mapData.info.origin.position.x+1;
                robotGoal.target_pose.pose.position.y = loc_y*mapData.info.resolution + mapData.info.origin.position.y;
            }
            ...
            std::cout << "initilize potentialmap" << std::endl;
            ...
            while(iteration < 2000 && minDis2Frontier > 1){
                ...
                if(iteration >= 1){
                    p.x=(path[path.size()-2])[0] * mapData.info.resolution + mapData.info.origin.position.x; 
                    p.y=(path[path.size()-2])[1] * mapData.info.resolution + mapData.info.origin.position.y;
                    p.z=0.0;

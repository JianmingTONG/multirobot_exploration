    ...
    while(ros::ok()){
...

        {
            ...
            while (idx_target < targets.size()) {
                ...
                loc_x = (targets[idx_target][1]+1)*mapData.info.resolution + mapData.info.origin.position.x;
                loc_y = targets[idx_target][0]*mapData.info.resolution + mapData.info.origin.position.y;
                index_costmap = (loc_y - costmapData.info.origin.position.y)/costmapData.info.resolution * costmapData.info.width + (loc_x - costmapData.info.origin.position.x)/costmapData.info.resolution;
                if (costmapData.data[index_costmap] >0){
                    targets.erase(targets.begin() + idx_target);
                    continue;
                }

                loc_x = targets[idx_target][1]*mapData.info.resolution + mapData.info.origin.position.x;
                loc_y = (targets[idx_target][0]+1)*mapData.info.resolution + mapData.info.origin.position.y;
                index_costmap = (loc_y - costmapData.info.origin.position.y)/costmapData.info.resolution * costmapData.info.width + (loc_x - costmapData.info.origin.position.x)/costmapData.info.resolution;
                if (costmapData.data[index_costmap] >0){
                    targets.erase(targets.begin() + idx_target);
                    continue;
              ...
              
        {
            ...
            int idx_obstacle = 0;
            while (idx_obstacle < obstacles.size()) {
                float dis = abs(obstacles[idx_obstacle][0] - startingLoc[0]) + abs(obstacles[idx_obstacle][1] - startingLoc[1]);
                if(dis > DISTANCE_THRES_VALID_OBSTACLE){
        ...
        while(iteration < 300 && minDis2Frontier > 1){
            // std::ofstream ofile_map;
            ...
            // ---------------- to keep robot following the APF to avoid targets jumping.
            if(iteration%50 == 49){
                robotGoal.target_pose.pose.orientation.z = 1;
                robotGoal.target_pose.pose.orientation.w = 0;
                robotGoal.target_pose.pose.position.x = (path.back())[1]*mapData.info.resolution + mapData.info.origin.position.x;
                robotGoal.target_pose.pose.position.y = (path.back())[0]*mapData.info.resolution + mapData.info.origin.position.y;

        ...
        if True:
            ...
            for i in range(len(results)):
                # if validmatch[i]:
                    ...
                    match_width = 4
                    match_num_thresh = 2
                    for matched_info in self.matched_list[:-1]:
                        if matched_info['robot1_id'] == self_id and matched_info['robot2_id'] == other_id:
                            if abs(matched_info['robot1_submap_index'] - self.matched_list[-1]['robot1_submap_index'])<match_width and abs(matched_info['robot2_submap_index'] - self.matched_list[-1]['robot2_submap_index'])<match_width :
                                print(matched_info)
                                matched_num += 1
                        if matched_info['robot2_id'] == self_id and matched_info['robot1_id'] == other_id:
                            if abs(matched_info['robot2_submap_index'] - self.matched_list[-1]['robot1_submap_index'])<match_width and abs(matched_info['robot1_submap_index'] - self.matched_list[-1]['robot2_submap_index'])<match_width :
                                print(matched_info)
                                matched_num += 1
                    if matched_num>=match_num_thresh:

...
int main(int argc, char **argv)
{
	...
	while (ros::ok()){
        ...
        if(frontiers.size()< 10){
            map_lengthX = costmapData.info.width*costmapData.info.resolution;
	        map_lengthY = costmapData.info.height*costmapData.info.resolution;
            // std::cout << "start rrt tree growing" << std::endl;
            // Sample free
            x_rand.clear();
            xr=(drand()*map_lengthX) + costmapData.info.origin.position.x;
            yr=(drand()*map_lengthY) + costmapData.info.origin.position.y;

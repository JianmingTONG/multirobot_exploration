...
def globalMap(data, args):
    ...
    ########################## new
    globalmaps[args] = data
    rospy.loginfo("receive globalmaps["+ str(args) +"]")
    ########################## changed by jianming
...
# ---------------------------------------------------------------------------------------------------------------
    ...
    rospy.loginfo("globalmaps initilization")
    print("globalmaps initilization")
    if len(namespace) > 0:
        for i in range(0, n_robots):
            # rospy.Subscriber(namespace+str(i+namespace_init_count) +
            #                  global_costmap_topic, OccupancyGrid, callback=globalMap, callback_args=i)
            rospy.Subscriber(namespace+str(i+namespace_init_count) +'/move_base/global_costmap/costmap', OccupancyGrid, callback=globalMap, callback_args=i)
    elif len(namespace) == 0:
        # rospy.Subscriber(global_costmap_topic, OccupancyGrid, globalMap)
        rospy.Subscriber(namespace+'1/move_base/global_costmap/costmap', OccupancyGrid, callback=globalMap, callback_args=0)
...
# wait if any of robots' global costmap map is not received yet
    for i in range(0, n_robots):
        while (len(globalmaps[i].data) < 1):
            rospy.loginfo('Waiting for the global costmap of robot '+ str(i+1))
            rospy.sleep(0.1)
            pass
    ...
    ########################### added by jianming
    # filteredPointStampListpub = rospy.Publisher('filtered_points_stamp_list', PointStampedArray, queue_size=10)
    ###########################
...
# Set the marker action for centroids.  Options are ADD, DELETE, and new in ROS Indigo: 3 (DELETEALL)
    ...
    ########################### added by jianming
    # arraypointstamps = PointStampedArray()
    # tempPointStamped = PointStamped()
    # tempPointStamped.header.frame_id = mapData.header.frame_id
...
# clearing old frontiers
        ...
        while z < len(centroids):
            ...
            for i in range(0, n_robots):
                # remove the goals which are in the obstacle
...
# publishing
        ...
        ########################### added by jianming
        # arraypointstamps.pointstamps = []
        # for q in range(0, len(centroids)):
        #     tempPointStamped.point.x = centroids[q][0]
        #     tempPointStamped.point.y = centroids[q][1]
        #     arraypointstamps.pointstamps.append(copy(tempPointStamped))
        # filteredPointStampListpub.publish(arraypointstamps)
        ###########################
# -------------------------------------------------------------------------

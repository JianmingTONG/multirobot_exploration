...
class robot:
    ...
    def __init__(self, name):
        ...
        ########################### added by jianming
        # rospy.loginfo(self.name + "movebase service is " + self.name+'/move_base')
        ###########################

        self.client.wait_for_server()
        # robot.goal.target_pose.header.frame_id = self.global_frame
        robot.goal.target_pose.header.frame_id = 'robot'+self.name[-1]+'/map'
        
        ########################### added by jianming   
        # rospy.loginfo("robot initialization frame: "+ robot.goal.target_pose.header.frame_id )
        ###########################
        ...
        while cond == 0:
            try:
                (trans, rot) = self.listener.lookupTransform(
                    self.global_frame, self.name+'/'+self.robot_frame, rospy.Time(0))
                ########################### added by jianming
                # rospy.loginfo(self.name+ " getPostion target frame " + self.global_frame + " source frame: " + self.name+'/'+self.robot_frame)
                ########################### 
                cond = 1
            except (tf.LookupException, tf.ConnectivityException, tf.ExtrapolationException):
                cond == 0
    ...
    def sendGoal(self, point):
        ############################### The key to avoid bugs
        robot.goal.target_pose.header.frame_id = 'robot'+self.name[-1]+'/map'
        ###############################
        rospy.loginfo(self.name + " send goal target frame: " + robot.goal.target_pose.header.frame_id + " point = (" + str(point[0]) +"," + str(point[1]) + ") " )
    ...
    # ########################### added by jianming
    # def sendGoalPointStamp(self, pointstamp):
    #     tfLisn = tf.TransformListener()
    #     transformedPoint = tfLisn.transformPoint(self.robot_frame, pointstamp)
    #     robot.goal.target_pose.pose.position.x = transformedPoint.point.x
    #     robot.goal.target_pose.pose.position.y = transformedPoint.point.y
    #     robot.goal.target_pose.pose.orientation.w = 1.0
    #     self.client.send_goal(robot.goal)
    #     point = [transformedPoint.point.x, transformedPoint.point.y]
    #     self.assigned_point = array(point)
    # ###########################

...
class robot:
    ...
    def __init__(self, name):
        self.assigned_point = []
        self.assigned_point_merged_map_frame = PoseStamped()
        self.name = name
        self.global_frame = rospy.get_param('~global_frame', 'robot1/map')
        self.robot_frame = rospy.get_param('~robot_frame', 'base_link')
        self.plan_service = rospy.get_param(
            '~plan_service', '/move_base_node/NavfnROS/make_plan')
        self.listener = tf.TransformListener()
        self.listener.waitForTransform(
            self.global_frame, self.name+'/'+self.robot_frame, rospy.Time(0), rospy.Duration(5.0))
        cond = 0
        while cond == 0:
            try:
        ...
        # rospy.loginfo("actionlib starts waiting")
        self.client.wait_for_server()
        # rospy.loginfo("actionlib ends waiting")

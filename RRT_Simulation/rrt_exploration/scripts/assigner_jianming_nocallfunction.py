...
def node():
    ...
    info_radius          = rospy.get_param('~info_radius',1.0)
    global_frame         = rospy.get_param('~global_frame',"robot1/map")
    robot_frame_suffix   = rospy.get_param('~robot_frame_suffix',"/map")
    ...
    if len(namespace)>0:
        for i in range(0,n_robots):
            ...
            robots.append(actionlib.SimpleActionClient('robot'+ str(i+1) +'/move_base', MoveBaseAction) )
            robots[i].wait_for_server()
            robotsGoal.append(PoseStamped())
    elif len(namespace)==0:
        robots.append(actionlib.SimpleActionClient('robot1/move_base', MoveBaseAction))
        robotsGoal.append(PoseStamped())
    ...
    while not rospy.is_shutdown():
        ...
        nb=[] #busy robots
        for i in range(0,n_robots):
            if (robots[i].get_state()==actionlib.SimpleGoalState.ACTIVE):
                #----------------------- whether the current goal is in obstacle    
                tempTransformedPoint = tfLisn.transformPoint(merged_costmap.header.frame_id, robotsGoal[i])
                ...
                tempTransformedPoint_map = tfLisn.transformPoint(mapData.header.frame_id, robotsGoal[i])
        ...
        robots_position = []
        for i in na:
            (trans, rot) = tfLisn.lookupTransform(global_frame, "robot" + str(i+1) + robot_frame_suffix, rospy.Time(0))
            robots_position.append(array([trans[0], trans[1]]))
        
        ip = 0
        for ir in na:
            while ip < len(centroids_process):
                if(linalg.norm(robots_position[ir] - centroids_process[ip]) < mini_step):
        ...
        for pointIdx in range(0, len(centroids_process)):
            ...
            for robotIdx in na:    
                ...
                robot_pos = robots_position[robotIdx]
                dis = linalg.norm(robot_pos - centroids_process[pointIdx])
        ...
        robotsGoal[id_record[winner_id]] = pointstamp
        goal.target_pose.header.frame_id = "robot" + str(id_record[winner_id]+1) + robot_frame_suffix
        goal.target_pose.pose.position.x = transformedPointStamp.point.x
        goal.target_pose.pose.position.y = transformedPointStamp.point.y
        goal.target_pose.pose.orientation.w = 1.0
        robots[id_record[winner_id]].send_goal(goal)

...
def node():
    global frontiers, mapData, costmapData, global1, global2, global3, globalmaps, litraIndx, n_robots, namespace_init_count
...
# wait if map is not received yet
    ...
    while (len(costmapData.data) < 1):
        rospy.loginfo('Waiting for the global map')
        rospy.sleep(0.1)
        pass
# wait if any of robots' global costmap map is not received yet
    for i in range(0, n_robots):
        while (len(globalmaps[i].data) < 1):
            rospy.loginfo('Waiting for the global costmap from each robot')
            rospy.sleep(0.1)
            pass
